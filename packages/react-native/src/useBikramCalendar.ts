/**
 * Deliberate sibling of the same hook in `@inicrea/bikram-sambat-react`.
 *
 * It is pure React glue over `@inicrea/bikram-sambat-core` with no DOM references, so
 * the two copies are identical. They are kept separate so neither UI package
 * has to depend on the other (an RN app pulling in react-dom as a peer would
 * be worse than this duplication). The calendar maths that must never drift
 * lives in the core package and is shared.
 */
